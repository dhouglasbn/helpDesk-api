...
export default class TicketController {
	...
	showClientHistory = async (request: OurRequest, reply: Response) => {
		if (!request.user?.role || request.user.role !== "client") {
			return reply.status(403).json({ message: "Acesso negado: Somente o cliente pode ver seu histórico de tickets." })
		}
	...
	listTechTickets = async (request: OurRequest, reply: Response) => {
		if (!request.user?.role || request.user.role !== "tech") {
			return reply.status(403).json({ message: "Acesso negado: Somente o técnico pode listar seus tickets." })
		}
	...
	listAllTickets = async (request: OurRequest, reply: Response) => {
		if (!request.user?.role || request.user.role !== "admin") {
			return reply.status(403).json({ message: "Acesso negado: Somente um admin pode listar todos os tickets." })
		}
	...
	addServicesToATicket = async (request: OurRequest, reply: Response) => {
		if (!request.user?.role || (request.user.role !== "tech" && request.user.role !== "admin")) {
			return reply
				.status(403)
				.json({ message: "Acesso negado: Somente técnicos e admins podem adicionar serviços a um chamado." })
		}

		try {
			const { ticketId } = request.params
			const { servicesIds } = request.body
			const services = await this.ticketService.addServicesToTicket(ticketId, request.user, servicesIds)
	...
	updateTicketStatus = async(request: OurRequest, reply: Response) => {
		if (!request.user?.role || (request.user.role !== "tech" && request.user.role !== "admin")) {
			return reply
				.status(403)
				.json({ message: "Acesso negado: Somente técnicos e admins podem atualizar o status de um chamado." })
		}

		try {
			const { ticketId } = request.params
			const { status } = request.body
			const ticket = await this.ticketService.updateStatus(ticketId, request.user, status)
