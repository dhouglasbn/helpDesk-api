...
export default class TicketService {
  ...
  listTechTickets = async(techId: string) =>  await db
    .select({
      id: schema.tickets.id,
      clientId: schema.tickets.clientId,
      techId: schema.tickets.techId,
    ...
    addServicesToTicket = async (ticketId: string, user: UserInToken, servicesIds: string[]) => {
      ...
      return await db.transaction(async (tx) => {
        await db.insert(schema.ticketServices)
          .values(
            servicesIds.map(serviceId => ({ ticketId, serviceId }))
          ).onConflictDoNothing()
