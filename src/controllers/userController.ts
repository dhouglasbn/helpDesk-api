...
export default class UserController {
	...
	createTechAccount = async (request: OurRequest, reply: Response) => {
		if (!request.user?.role || request.user.role !== "admin") {
			return reply.status(403).json({
				message: "Acesso negado: Somente o admin pode criar contas de técnicos.",
			})
		}
	...
	listClientAccounts = async (request: OurRequest, reply: Response) => {
		if (!request.user?.role || request.user.role !== "admin") {
			return reply.status(403).json({
				message: "Acesso negado: Somente o admin pode listar contas de clientes.",
			})
		}
	...
	updateUserPicture = async (request: OurRequest, reply: Response) => {
		if (request.user?.id && request.user.id !== request.params.id && request.user.role !== "admin") {
			return reply.status(403).json({
				message: "Acesso negado: Você só pode atualizar a foto do seu próprio perfil.",
			})
		}
		try {
			if (request.file) {
	...
	updateTechAccount = async (request: OurRequest, reply: Response) => {
		...
		if (!isUserAuthorized) {
			return reply.status(403).json({
				message: "Acesso negado: Somente técnicos e admins podem atualizar contas de técnicos.",
			})
		}
		if (request.user?.id && request.user.id !== request.params.id && request.user.role !== "admin") {
			return reply.status(403).json({
	...
	updateAdminAccount = async (request: OurRequest, reply: Response) => {
		...
		if (!isUserAuthorized) {
			return reply.status(403).json({
				message: "Acesso negado: Somente admins podem atualizar contas de admins.",
			})
		}
		try {
			const updatingUserId = request.params.id
	...
	updateClientAccount = async (request: OurRequest, reply: Response) => {
		...
		if (!isUserAuthorized) {
			return reply.status(403).json({
				message: "Acesso negado: Somente clientes e admins podem atualizar contas de clientes.",
			})
		}
		if (request.user?.id && request.user.id !== request.params.id && request.user.role !== "admin") {
			return reply.status(403).json({
	...
	deleteClientAccount = async (request: OurRequest, reply: Response) => {
		...
		if (!isUserAuthorized) {
			return reply.status(403).json({
				message: "Acesso negado: Somente admins e clientes podem deletar contas de clientes.",
			})
		}
		if (request.user?.id && request.user.id !== request.params.id && request.user.role !== "admin") {
			return reply.status(403).json({
